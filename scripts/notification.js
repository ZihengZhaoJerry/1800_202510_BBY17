...
async function displayNotifications() {
    ...
    // 1. Get all user's posts
    const postsSnapshot = await db.collection("posts")
        .where("owner", "==", user.uid)
        .get();

    // 2. Collect all unread comments
    const unreadComments = [];
    for (const postDoc of postsSnapshot.docs) {
        const postData = postDoc.data();
        ...
        commentsSnapshot.forEach(commentDoc => {
            ...
            unreadComments.push({
                postId: postDoc.id,
                postTitle: postData.title || "Untitled Post",
                postContent: postData.content || "No content",
    ...
    unreadComments.sort((a, b) => b.timestamp?.toMillis() - a.timestamp?.toMillis());
    ...
    unreadComments.forEach(comment => {
        ...
        card.innerHTML = `
            <div class="notification-content">
                ...
                <p><strong>Post Content:</strong> ${comment.postContent.substring(0, 50)}...</p>
                <p><strong>Comment:</strong> ${comment.commentText}</p>
                <p><strong>From:</strong> ${comment.commenterName}</p>
                <small>${comment.timestamp?.toDate().toLocaleString() || "Time unknown"}</small>
    ...
    document.querySelectorAll(".view-post-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            window.location.href = `inside_post.html?postId=${e.target.dataset.postId}`;
        });
    });
    ...
    notificationsContainer.prepend(markAllBtn);
}
